...
def test_contextual_review_uses_repository_context(tmp_path: Path) -> None:
    ...
    assert payload["context"]["module_count"] >= 1
    assert payload["context"]["file_count"] >= 1
    assert any("context" in finding["message"].lower() for finding in payload["findings"])
